...
function ListingContext({children}) {
    ...
    const handleAddListing = async () => {
        ...
        try {
            ...
            setAdding(false)
            console.log(error)
            toast.error(error.response?.data?.message || "AddListing error")
        }
    }
    ...
    // FIX: Search API blank input pe call na ho, and use encodeURIComponent
    const handleSearch = async (data) => {
        if (!data || data.trim() === "") {
            setSearchData([]); // empty input pe suggestion clear karo
            return;
        }
        try {
            let result = await axios.get(serverUrl + `/api/listing/search?query=${encodeURIComponent(data)}`)
